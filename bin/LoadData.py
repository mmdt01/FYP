...
class LoadMyData(LoadData):
    '''Subclass of LoadData for loading my already preprocessed data .fif file'''
    def __init__(self, file_to_load,*args):
        self.file_to_load = file_to_load
        super(LoadMyData,self).__init__(*args)

    def get_epochs(self, tmin=0, tmax=3, baseline=None):
        # read the raw data
        raw_data = mne.io.read_raw_fif(self.eeg_file_path + '/' + self.file_to_load, preload=True)
        ...
        events, event_ids = mne.events_from_annotations(raw_data, event_id=event_dict)
        epochs = mne.Epochs(raw_data, events, event_id=[5,6], tmin=tmin, tmax=tmax,
        ...
        # normalize the labels ensuring they start from 0
